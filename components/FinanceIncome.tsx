  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          type="button"
          className="rounded-2xl border border-sky-600 bg-sky-600/10 px-4 py-3 text-sm font-semibold text-sky-300 transition hover:bg-sky-600/20"
        >
          + Καταχώρηση Εσόδου
        </button>
      </div>
          ...
          <tbody>
            {incomeTransactions.map((transaction) => (
              <tr key={transaction.id} className="border-b border-zinc-800 hover:bg-zinc-900/60">
                ...
                <td className="px-4 py-4 text-sm text-zinc-200">{formatRelatedValue('Πρακτορείο', transaction.agency_id)}</td>
                <td className="px-4 py-4 text-sm text-zinc-200">{formatRelatedValue('Αντιπρόσωπος', transaction.representative_id)}</td>
